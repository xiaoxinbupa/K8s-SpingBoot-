...
@Configuration
public class ContentDetailAssemble {

    public ContentDetailVO assembleDtoToVo(ContentDetailDTO source) {
        if (source == null) {
            return null;
        }
        ContentDetailVO target = new ContentDetailVO();
        target.setCommentList(source.getCommentList());
        target.setContentDTO(source.getContentDTO());
        return target;
    }
}
