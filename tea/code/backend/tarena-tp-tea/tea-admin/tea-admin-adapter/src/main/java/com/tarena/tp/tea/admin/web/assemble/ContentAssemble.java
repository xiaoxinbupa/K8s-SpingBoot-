...
public class ContentAssemble {
    ...
    public ContentVO assembleDtoToVo(ContentDTO source) {
        if (source == null) {
            return null;
        }
    ...
    public List<ContentVO> assembleDtosToVos(List<ContentDTO> sources) {
        if (sources == null) {
            return null;
        }
        List<ContentVO> targets = new ArrayList<>();
        for (ContentDTO source : sources) {
            ContentVO target = new ContentVO();
            BeanUtils.copyProperties(source,target);
            targets.add(target);
        }
